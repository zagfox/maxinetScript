...
class thinFT(Topo):
    ...
    def __init__(self, hosts=2, bwlimit=10, lat=0.1, **opts):
        ...
        for i in range(0, numLeafes, 2):
            h1 = self.addHost('h' + str(i+1), mac=self.makeMAC(i), ip="10.0.0." + str(i+1))
            sw = self.addSwitch('s' + str(s), dpid=self.makeDPID(s),  **dict(listenPort=(13000+s-1)))
            s = s+1
            self.addLink(h1, sw, bw=bw, delay=str(lat) + "ms")

            if i!=numLeafes-1:
                h2 = self.addHost('h' + str(i+2), mac=self.makeMAC(i), ip="10.0.0." + str(i+2))
                self.addLink(h2, sw, bw=bw, delay=str(lat) + "ms")
            tor.append(sw)
